...
private static void toByte(String s){
    if(s==null){
        return ;
    }
    ...
    System.out.print("s:"+"a".getBytes().length);
    byte[] b= new byte[10];
    for(int i=0;i<s.length();i++){
        String ss = String.valueOf(s.charAt(i));
        byte[] sb = ss.getBytes();
        String sb1= new String(sb);
        System.out.println("ss:"+ss+"   "+ss.getBytes().length);
        System.out.println("s:"+"a".getBytes().length);
        b[i]= ss.getBytes()[0];
    }
